...
function mapTourService(s: Record<string, unknown>, t: (key: string) => string): Service {
  ...
  const providerName = provider
    ? (provider.company as { name: string } | undefined)?.name || `${provider.firstName || ""} ${provider.lastName || ""}`.trim()
    : undefined;
  return {
    id: s.id as string,
...
export default function ToursClient({ initialData }: ToursClientProps) {
  const { t } = useI18n();
  const searchParams = useSearchParams();
  const [tourCategory, setTourCategory] = useState("");
  const [tourCounts, setTourCounts] = useState<{ all: number; oneDay: number; multiDay: number }>({ all: initialData?.totalCount || 0, oneDay: Math.floor((initialData?.totalCount || 0) * 0.3), multiDay: Math.ceil((initialData?.totalCount || 0) * 0.7) });
